...
public partial class RfidConnectApiForSharif : HttpClientHandler
{
    private readonly IConfiguration _configuration;
    private readonly ILogger<RfidConnectApiForSharif> _logger;
    private string baseUri;

    public RfidConnectApiForSharif(IConfiguration configuration, ILogger<RfidConnectApiForSharif> logger)
    {
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        try
        {
            var token = _configuration["RfidConnectApi:Token"];
        ...
        if (!response.IsSuccessStatusCode)
        {
            try
            {
                if (response.Content?.Headers?.ContentType?.MediaType == "application/json")
                {
                    ...
                    _logger.LogWarning("Auth log error: {ErrorResult}", errorResult);
                }
                else
                {
                    _logger.LogWarning("HTTP error {StatusCode} returned for request {Method} {Uri}", response.StatusCode, request.Method, request.RequestUri);
                }

                if (response.StatusCode == HttpStatusCode.Unauthorized)
                {
                    _logger.LogWarning("Received Unauthorized (401) from API.");
                }

                if (response.StatusCode == HttpStatusCode.Ambiguous)
                {
                    _logger.LogWarning("Received Ambiguous (300) from API.");
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error processing error response: {Message}", ex.Message);
            }
        }
    ...
    private void SetUri()
    {
        ...
        {
            baseUri = $"http://{_configuration.GetSection("RfidConnectApi")["Ip"]}/RfidCore/v2/";
        }
        else
        {
            baseUri = $"http://{_configuration.GetSection("RfidConnectApi")["Ip"]}{uri.Value}";
        }
    }
}
