...
public class Hid : object
{
    ...
    public HID_RETURN OpenDevice(ushort vID, ushort pID, string serial)
    {
        if (deviceOpened == false)
        {
            ...
            for (int i = 0; i < deviceList.Count; i++)
            {
                ...
                nint device = CreateFile(deviceList[i],
                                           DESIREDACCESS.GENERIC_READ | DESIREDACCESS.GENERIC_WRITE,
                                            1 | 2,
                                            0,
                                            CREATIONDISPOSITION.OPEN_EXISTING,
                ...
                if (device != INVALID_HANDLE_VALUE)
                {
                    ...
                    nint serialBuff = Marshal.AllocHGlobal(512);
    ...
    public static void GetHidDeviceList(ref List<string> deviceList)
    {
        ...
        nint hidInfoSet = SetupDiGetClassDevs(ref hUSB, 0, nint.Zero, DIGCF.DIGCF_PRESENT | DIGCF.DIGCF_DEVICEINTERFACE);
        if (hidInfoSet != nint.Zero)
        {
            ...
            {
                //得到第index个接口信息
                if (SetupDiEnumDeviceInterfaces(hidInfoSet, nint.Zero, ref hUSB, index, ref interfaceInfo))
                {
                    int buffsize = 0;
                    // 取得接口详细信息:第一次读取错误,但可以取得信息缓冲区的大小
                    SetupDiGetDeviceInterfaceDetail(hidInfoSet, ref interfaceInfo, nint.Zero, buffsize, ref buffsize, null);
                    //构建接收缓冲
                    nint pDetail = Marshal.AllocHGlobal(buffsize);
                    ...
                    {
                        deviceList.Add(Marshal.PtrToStringAuto((int)pDetail + 4));
                    }
