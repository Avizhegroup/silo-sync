...
*********************************************************************************************************/
    ...
    *********************************************************************************************************/
    ...
    public delegate void OnDataReceived(nint epc, short recvLen);//[MarshalAs(UnmanagedType.LPArray, SizeConst = 4096)]
    ...
    private static void recvDataCallback(nint epc, short recvLen)
    {
    ...
    public string GetHardwareVersion()
    {
        ...
        if (UHFGetHardwareVersion(version) == 0)
        {
            ...
            return Encoding.ASCII.GetString(versionTemp);// DataConvert.ByteArrayToHexString(versionTemp);
        }
        return string.Empty;
    }
    /// <summary>
    ...
    public bool uhfGetReceived(ref string epc, ref string tid, ref string rssi, ref string ant)
    {
        ...
        if (GetReceived_EX(ref uLen, ref bufData))
        {
            ...
            rssi_data = (rssiTemp / 10.0).ToString();// RSSI  =  (0xFED6   -65535)/10
    ...
    public string GetAPIVersion()
    {
        ...
        return "Ver" + result + ".0 (" + Encoding.ASCII.GetString(time, 0, time.Length).Replace("\0", "") + ")";
    ...
    public List<DeviceInfo> LinkGetDeviceInfo()
    {
        ...
        if (resultLen > 0)
        {
            string jsonstring = Encoding.ASCII.GetString(info, 0, resultLen).Replace("\0", "");
    ...
    public TagInfo getTagData()
    {
        ...
        if (result > 0)
        {
            ...
            while (true)
            {
                ...
                byte[] data = Utils.CopyArray(tagTempData, index, len);
                index = index + len;
                ...
                else if (type == 4)
                {
                    ...
                    string rssi_data = (rssiTemp / 10.0).ToString();// RSSI  =  (0xFED6   -65535)/10
                    if (!rssi_data.Contains("."))
                    {
                        rssi_data = rssi_data + ".0";
                    }
